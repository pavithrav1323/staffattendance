import { generateAuthenticationOptionsService, generateRegistrationOptionsService, hasBiometricCredentials, verifyAuthenticationService, verifyRegistrationService, } from "../modules/webauthn/webauthn.service.js";
/**
 * Check if user has biometric credentials
 */
export const checkBiometricCredentials = async (req, res, next) => {
    try {
        if (!req.user) {
 ...
 */
export const generateRegistrationOptions = async (req, res, next) => {
    try {
        if (!req.user) {
            res.status(401).json({ success: false, message: "Unauthorized" });
            return;
 ...
 */
export const generateAuthenticationOptions = async (req, res, next) => {
    try {
        if (!req.user) {
            res.status(401).json({ success: false, message: "Unauthorized" });
            return;
