  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "Latitude, longitude, and accuracy are required",
      });
    }
    ...
    const todayString = getDateInTimeZone(
      userData.companyTimezone
    );
    ...
    const existingAttendance = await db
      ...
      .where(
        and(
          ...
          eq(attendance.attendanceDate, todayString),
          eq(
    ...
    const insertData: NewAttendance = {
    ...
    return res.status(500).json({
      success: false,
      message:
        "Internal server error during clock in",
    });
  }
};
  ...
  try {
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        clockOutTime:
          updatedAttendance.clockOutTime,
    ...
    return res.status(500).json({
      success: false,
      message:
        "Internal server error during clock out",
    });
  }
};
  ...
  try {
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        clockInTime:
          activeAttendance[0].clockInTime,
...
export const getAttendanceHistory = async (
  ...
  try {
    ...
    const rangeResult = await resolveStaffDateRange(
      companyId,
      reportType,
      date,
      month,
    ...
    const history = await db
      .select({
        ...
        clockInLocationName:
          attendance.clockInLocationName,
        ...
        clockOutLocationName:
          attendance.clockOutLocationName,

        clockOutLocationStatus:
          attendance.clockOutLocationStatus,
 ...
 */
export const exportAttendanceHistory = async (
  req: AuthRequest,
  res: Response
) => {
  try {
    ...
    console.error(
      "Export attendance history error:",
      error
    );
 ...
 */
async function resolveStaffDateRange(
  companyId: string,
  reportType?: string,
  date?: string,
  month?: string,
  ...
  | { ok: true; dateStart: string; dateEnd: string; reportLabel: string }
...
async function getCurrentDatePartForCompany(
  companyId: string,
  ...
  try {
    ...
    const [attendanceSummary] = await db
      .select({
        ...
        totalWorkingMinutes: sql<number>`
    ...
    return res.status(200).json({
      ...
      data: {
        month,
        presentDays:
          Number(attendanceSummary?.presentDays ?? 0),
        totalWorkingMinutes:
          Number(
            attendanceSummary?.totalWorkingMinutes ?? 0
          ),
      },
    });
  } catch (error) {
