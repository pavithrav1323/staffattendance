...
import { generateClinicalReportDocx } from "./clinical-reports.docx-converter.js";
...
export function getReportNumberPrefix(companyCode: string): string {
  const match = String(companyCode ?? "").trim().match(/(\d)$/);
  if (!match) {
    throw new AppError(
      400,
      "Unable to generate Clinical Report ID: company code is missing or does not end with a digit"
    );
  }
  return match[1];
}
...
export async function createClinicalReport(
  ...
  const prefix = getReportNumberPrefix(company.companyCode);
...
export async function deleteClinicalReports(
  authUser: AuthUser,
  reportIds: string[]
  ...
  if (!reportIds.length) {
    throw new AppError(400, "At least one Clinical Report must be selected");
  }
  ...
  const conditions = [
    eq(clinicalReports.companyId, authUser.companyId),
    eq(clinicalReports.isDeleted, false),
    inArray(clinicalReports.id, uniqueReportIds),
  ];

  if (authUser.role === "STAFF") {
    conditions.push(eq(clinicalReports.submittedBy, authUser.userId));
  } else if (authUser.role !== "ADMIN" && authUser.role !== "MASTER_ADMIN") {
    throw new AppError(403, "Access denied");
  }

  const matchingReports = await db
    .select({ id: clinicalReports.id })
    .from(clinicalReports)
    .where(and(...conditions));

  if (matchingReports.length !== uniqueReportIds.length) {
    throw new AppError(
      403,
      "You do not have permission to delete one or more selected Clinical Reports"
    );
  }
  ...
  return { deletedCount: matchingReports.length };
}
