...
export async function getPendingStaff(authUser: AuthUser) {
...
export async function getAdminAttendance(
  ...
  if (employeeId) {
    ...
    if (adminDepartmentId) {
      employeeConditions.push(
        eq(users.departmentId, adminDepartmentId)
      );
    }
  ...
  if (adminDepartmentId) {
    conditions.push(
      eq(attendance.departmentId, adminDepartmentId)
    );
  }
...
export async function getAdminAttendanceExport(
  authUser: AuthUser,
  reportType?: string,
  date?: string,
  month?: string,
 ...
 */
async function resolveAdminDateRange(
  authUser: AuthUser,
  reportType?: string,
  date?: string,
  month?: string,
...
async function getCurrentDatePartForAdmin(
  authUser: AuthUser,
...
export async function getMasterAdminDashboardStats(
  authUser: AuthUser
) {
  if (!authUser.companyId) {
    throw new AppError(403, "Company context is required");
...
async function updateStaffStatus(
  ...
  if (!updatedUser) {
    throw new AppError(
      404,
      "Pending staff member not found"
    );
  }
  ...
  if (status === "APPROVED") {
    await db
      ...
      .where(
        and(
          eq(staffDevices.userId, staffId),
          eq(staffDevices.status, "PENDING")
        )
      );
  }
...
export async function activateStaff(
  ...
  return updateExistingStaffStatus(
    authUser,
    staffId,
    "APPROVED"
  );
...
export async function resetStaffDevice(
  authUser: AuthUser,
  staffId: string
) {
  if (!authUser.companyId) {
...
async function updateExistingStaffStatus(
  authUser: AuthUser,
  staffId: string,
...
export async function deleteStaff(
  ...
  await db
    ...
    .where(
      and(
        eq(attendance.employeeId, staffId),
...
export async function getDeletedStaff(
  authUser: AuthUser
) {
  if (!authUser.companyId) {
    throw new AppError(403, "Company context is required");
...
export async function getDeletedStaffAttendance(
  authUser: AuthUser,
  employeeId: string
) {
  if (!authUser.companyId) {
...
export async function deleteDeletedStaffAttendance(
  authUser: AuthUser,
  employeeId: string
) {
  if (!authUser.companyId) {
